...
function abc(){
  ...
  alert("要过自己想要的生活！");           }
function you(){
  var answer=prompt("请输入一个美好的生日愿望:(つ´ω`)つ");
  if(answer == null){
    alert("你没有填:)")
  }
  if(answer == ""){
  ...
  if (answer!= "" && answer != null) {
    var d = 0;
    var a = confirm("确定认真填了么？(ﾉ>ω<)ﾉ");
    if(!a){return false}
    if(a){//var b = confirm("是的，请点击进行下一步");
            //if(!b){return false};
            //if(b){  
              var c = confirm("已将你的愿望存入许愿池!给我发红包可以增加愿望实现概率｡◕∀◕｡ 是否发红包？");
                    if(c){alert("你期许的一定会到来！(*´ω`)人(´ω`*)")}
                    if(!c){alert("。。。(／‵Д′)／~ ╧╧")}
                      //}
  }           }
}
function showfirst(){
        document.getElementById("audio1").volume = 0.01;
        alert(document.getElementById("audio1").volume);
        document.getElementById("myVid").src="vid/vid1.mp4";
        document.getElementById("myVid").style.visibility="visible";
}
function showsecond(){
        document.getElementById("audio1").volume = 0.01;
        document.getElementById("myVid").src="vid/vid2.mp4";
        document.getElementById("myVid").style.visibility="visible";
}
function showthird(){
        document.getElementById("audio1").volume = 0.01;
        document.getElementById("myVid").src="vid/vid3.mp4";
        document.getElementById("myVid").style.visibility="visible";
}
function showfourth(){
        document.getElementById("audio1").volume = 0.1;
        document.getElementById("myVid").src="vid/vid4.mp4";
        document.getElementById("myVid").style.visibility="visible";
}